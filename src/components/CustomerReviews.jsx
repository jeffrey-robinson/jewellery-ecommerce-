  ...
  return (
    <div className="bg-[#FCF8F2] min-h-screen text-[#211522] pb-24">
      ...
      <section className="bg-[#3B183F] text-white py-20 relative overflow-hidden reveal-element border-b border-[#D4AF65]/20">
        ...
        <div className="pointer-events-none absolute -top-20 -right-10 h-72 w-72 rounded-full bg-[#6A3578]/10 blur-3xl animate-drift-slow" />
      ...
      <section className="max-w-7xl mx-auto px-5 sm:px-8 mt-16 lg:mt-24 reveal-element">
        <div className="bg-white rounded-[2rem] p-6 sm:p-10 border border-[#E8D8EE] shadow-soft grid md:grid-cols-[1fr_1.5fr] gap-10 items-center">
          {/* Average Rating Card */}
          <div className="text-center md:border-r border-[#E8D8EE]/60 md:pr-10 py-4 flex flex-col items-center">
            <span className="text-xs font-semibold tracking-wider text-[#211522]/40 uppercase">Average Rating</span>
            <div className="font-display text-6xl sm:text-7xl text-[#211522] font-bold mt-2 mb-4">4.9</div>
            <div className="flex items-center gap-0.5 mb-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} size={18} className="fill-[#D4AF65] text-[#D4AF65]" />
              ))}
            </div>
            <span className="text-xs text-[#211522]/50 font-body">Based on {totalReviews} verified purchases</span>
          </div>

          {/* Progress Bars breakdown */}
          <div className="space-y-3.5">
            <h4 className="text-xs font-semibold tracking-wider text-[#211522]/40 uppercase mb-4">Rating Breakdown</h4>
            {ratingDistribution.map((row) => (
              <div key={row.stars} className="flex items-center gap-4 text-xs font-body">
                <span className="w-12 text-[#211522]/60 font-medium flex items-center gap-1">{row.stars} Stars</span>
                <div className="flex-grow h-2 bg-[#FCF8F2] rounded-full overflow-hidden border border-[#E8D8EE]/40">
                  <div 
                    className="h-full bg-[#6A3578] rounded-full" 
                    style={{ width: `${row.percentage}%` }}
                  />
                </div>
                <span className="w-12 text-right text-[#211522]/50">{row.percentage}% ({row.count})</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-5 sm:px-8 mt-16 lg:mt-24 reveal-element">
        <div className="border-b border-[#E8D8EE] pb-6 mb-12 flex justify-between items-end">
          <div>
            <h2 className="font-display text-2xl text-[#211522] font-semibold border-b-0">Featured Stories</h2>
            <p className="text-xs text-[#211522]/50 mt-1 font-body">Hand-verified purchase reviews from JEM owners</p>
          </div>
          <span className="text-xs uppercase tracking-wider text-[#6A3578] font-bold flex items-center gap-1.5">
            <MessageSquare size={13} className="text-[#D4AF65]" /> {expandedReviews.length} Stories Featured
          </span>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 perspective-1000">
          {expandedReviews.map((rev) => (
            <div 
              key={rev.id} 
              className="bg-white rounded-[2rem] p-6 sm:p-8 border border-[#E8D8EE] hover:border-[#D4AF65]/40 premium-3d-card shadow-soft transition-colors duration-500 flex flex-col justify-between"
            >
              <div>
                {/* Header: User avatar + info */}
                <div className="flex items-center gap-3.5 mb-5">
                  <div className="h-11 w-11 rounded-full overflow-hidden bg-[#FCF8F2] border border-[#E8D8EE] shrink-0">
                    <img src={rev.img} alt={rev.name} className="w-full h-full object-cover" />
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-[#211522] leading-tight">{rev.name}</h4>
                    <span className="text-[10px] uppercase tracking-wider text-[#6A3578] font-bold flex items-center gap-1 mt-0.5">
                      <Check size={10} strokeWidth={3} className="text-[#D4AF65]" /> {rev.role || 'Verified Buyer'}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-0.5 mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star 
                      key={i} 
                      size={12} 
                      className={i < rev.rating ? 'fill-[#D4AF65] text-[#D4AF65]' : 'text-[#211522]/15'} 
                    />
                  ))}
                </div>

                {/* Review Text */}
                <p className="text-xs sm:text-sm text-[#211522]/75 leading-relaxed font-body italic mb-6">
                  "{rev.text}"
                </p>
              </div>

              {/* Footer details: Product & date */}
              <div className="border-t border-[#E8D8EE]/60 pt-4 mt-4 flex items-center justify-between text-[11px] text-[#211522]/40 font-body">
                <span className="truncate max-w-[170px] font-semibold text-[#6A3578] uppercase tracking-wider text-[10px]">
      ...
      <section className="max-w-7xl mx-auto px-5 sm:px-8 mt-24">
        <div className="bg-[#3B183F] text-white rounded-[2rem] p-8 sm:p-12 border border-[#D4AF65]/20 shadow-soft relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div className="absolute inset-0 bg-[radial-gradient(#D4AF65_1px,transparent_1px)] [background-size:32px_32px] opacity-5 pointer-events-none" />
          <div className="pointer-events-none absolute -bottom-16 -left-10 h-56 w-56 rounded-full bg-[#6A3578]/10 blur-3xl animate-drift" />
          ...
          <button 
            ...
            className="relative z-10 px-8 py-4 rounded-full bg-[#6A3578] hover:bg-[#3B183F] border border-[#D4AF65]/35 hover:border-[#D4AF65] text-white text-xs font-semibold uppercase tracking-widest transition-all inline-flex items-center gap-2 shrink-0 active:scale-95 duration-100 shadow-md"
          >
            Submit a Review <Edit3 size={13} className="text-[#D4AF65]" />
          </button>
        </div>
      </section>
    </div>
