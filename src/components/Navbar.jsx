...
const links = ['Rings', 'Necklaces', 'Earrings', 'Bracelets', 'Journal']
    ...
    <header className="sticky top-0 z-50 bg-ivory/90 backdrop-blur border-b border-ink/10">
          ...
          <nav className="hidden lg:flex items-center gap-9">
            {links.map((l) => (
              <a
                key={l}
                href="#products"
                className="text-sm font-medium text-ink/70 hover:text-emerald transition-colors relative py-1 after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-0 after:bg-gold after:transition-all hover:after:w-full"
              >
                {l}
              </a>
            ))}
      ...
      {open && (
        <nav className="lg:hidden border-t border-ink/10 bg-ivory px-5 py-4 flex flex-col gap-4">
          {links.map((l) => (
            <a key={l} href="#products" onClick={() => setOpen(false)} className="text-base font-medium text-ink/80 hover:text-emerald">
              {l}
            </a>
          ))}
        </nav>
