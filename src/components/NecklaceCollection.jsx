  ...
  return (
    <div className="bg-ivory min-h-screen text-ink pb-24">
      ...
      <section className="bg-ink text-ivory py-20 relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute -top-16 -left-10 h-64 w-64 rounded-full bg-ruby/10 blur-3xl animate-drift-slow" />
        <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10 text-center">
          ...
          <p className="mt-4 text-sm text-ivory/65 max-w-lg mx-auto font-body leading-relaxed">
            Discover JEM's masterfully cast necklaces and pendants. Finished with gold-plated anti-tarnish protection and hand-set stones.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-5 sm:px-8 mt-16 lg:mt-24">
        <div className="border-b border-ink/10 pb-6 mb-12 flex justify-between items-end">
          <div>
            <h2 className="font-display text-2xl text-ink">Explore Pendants & Chains</h2>
            <p className="text-xs text-ink/50 mt-1 font-body">Premium finishes crafted for daily wear and structural brilliance</p>
          </div>
          <span className="text-xs uppercase tracking-wider text-gold font-semibold">
            {necklaceItems.length} Designs
          </span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 perspective-1000">
          {necklaceItems.map((product) => {
                  ...
                  <div className="space-y-2">
                    ...
                    {product.colors ? (
                      <div className="flex items-center gap-1.5 pt-1">
                        <span className="text-[10px] uppercase font-semibold text-ink/40">Colours:</span>
                        <div className="flex gap-1">
                          {product.colors.map(col => (
                            <span key={col} className="px-2 py-0.5 text-[9px] border border-ink/10 rounded-full font-body bg-ivory text-ink/70">
                              {col}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    ...
                    <div className="flex gap-1.5">
                      {selectedProduct.colors.map(col => (
                        <span key={col} className="px-2.5 py-0.5 text-[10px] border border-ink/10 rounded-full font-body bg-white text-ink/80">
                          {col}
                        </span>
                      ))}
                    </div>
